...
(function injectConfirmModal() {
    ...
    style.textContent = `
        #inf-confirm-overlay { display:none; position:fixed; inset:0; background:rgba(10,20,40,0.45); backdrop-filter:blur(6px); z-index:99999; align-items:center; justify-content:center; }
        ...
        .inf-confirm-box { background:rgba(255,255,255,0.97); backdrop-filter:blur(20px); border-radius:10px; width:420px; max-width:calc(100vw - 32px); overflow:hidden; position:relative; box-shadow:0 24px 60px rgba(27,79,168,0.15); animation:infBoxIn 0.35s cubic-bezier(0.16,1,0.3,1) both; }
        @keyframes infBoxIn { from{opacity:0;transform:scale(0.94) translateY(12px)} to{opacity:1;transform:none} }
        .inf-confirm-box::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,transparent,#F5911E,#1B4FA8,transparent); }
        .inf-confirm-icon { display:flex; align-items:center; justify-content:center; padding:32px 32px 0; }
        .inf-confirm-icon-wrap { width:52px; height:52px; border-radius:50%; background:rgba(245,145,30,0.08); border:1px solid rgba(245,145,30,0.2); display:flex; align-items:center; justify-content:center; position:relative; }
        .inf-confirm-icon-pulse { position:absolute; inset:-8px; border-radius:50%; border:1px solid rgba(245,145,30,0.15); animation:infPulse 2s ease-in-out infinite; }
        ...
        .inf-confirm-cancel { flex:1; max-width:140px; padding:11px 20px; background:transparent; border:1px solid rgba(27,79,168,0.15); border-radius:4px; color:#7A8A9A; font-family:'DM Sans',sans-serif; font-size:11px; letter-spacing:2px; text-transform:uppercase; cursor:pointer; transition:all 0.2s; }
        .inf-confirm-cancel:hover { border-color:rgba(27,79,168,0.3); color:#1A2A4A; }
        .inf-confirm-ok { flex:1; max-width:180px; padding:11px 20px; background:transparent; border:1.5px solid #1B4FA8; border-radius:4px; color:#1B4FA8; font-family:'Bebas Neue',sans-serif; font-size:14px; letter-spacing:4px; cursor:pointer; position:relative; overflow:hidden; transition:color 0.35s; }
        .inf-confirm-ok::before { content:''; position:absolute; inset:0; background:linear-gradient(90deg,#1B4FA8,#2D6FDB); transform:scaleX(0); transform-origin:left; transition:transform 0.35s cubic-bezier(0.16,1,0.3,1); }
        .inf-confirm-ok:hover::before { transform:scaleX(1); }
        .inf-confirm-ok:hover { color:#fff; }
        .inf-confirm-ok span { position:relative; z-index:1; }
        .inf-confirm-footer { padding:12px 32px; border-top:1px solid rgba(27,79,168,0.06); display:flex; align-items:center; gap:6px; font-family:'DM Sans',sans-serif; font-size:10px; color:#C8D4E0; }
    `;
    document.head.appendChild(style);
    const overlay = document.createElement('div');
    overlay.id = 'inf-confirm-overlay';
    overlay.innerHTML = `<div class="inf-confirm-box">
        <div class="inf-confirm-icon"><div class="inf-confirm-icon-wrap"><div class="inf-confirm-icon-pulse"></div>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#F5911E" stroke-width="1.5"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
        </div></div>
        <div class="inf-confirm-body">
            <div class="inf-confirm-label" id="inf-confirm-label">Confirm</div>
        ...
        <div class="inf-confirm-footer"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>Infinity Academy</div>
    </div>`;
...
const infConfirm = {
    _resolve: null,
    show({ label='Confirm', title='Are you sure?', message='This action cannot be undone.', okText='Confirm' } = {}) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let materialCache = {};
    let materialFetchController = null;
    ...
    function calculatePrice() {
        ...
        fetch('/calculate-price', {
            ...
            body:JSON.stringify({ type:typeInput.value, course_template_id:course.value, level_id:level.value, sublevel_id:sublevel.value, bundle_id:bundle?.value||null })
    ...
    function refreshPaymentSummary() {
        ...
        const sel           = paymentSelect.options[paymentSelect.selectedIndex];
        const depositPct    = parseFloat(sel.dataset.deposit     || 0);
        const installments  = parseInt(sel.dataset.installments  || 0);
        const grace         = parseInt(sel.dataset.grace         || 0);
        const needsApproval = sel.dataset.approval == 1;
        ...
        if (pricing.materialPrice > 0) html += infPayRow('Material (full)', fmt(pricing.materialPrice));
        if (pricing.testFee       > 0) html += infPayRow('Test Fee (full)',  fmt(pricing.testFee));
        const dueNow = depositAmt + pricing.materialPrice + pricing.testFee;
        html += `<div style="height:1px;background:rgba(27,79,168,0.08);margin:8px 0;"></div>`;
        html += infPayRow('Due Now', fmt(dueNow), 'accent');
        if (needsApproval) html += `<div style="display:inline-flex;align-items:center;gap:5px;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#92400E;background:rgba(245,145,30,0.12);border:1px solid rgba(245,145,30,0.25);border-radius:3px;padding:4px 9px;margin-top:8px;">⚠ Requires Admin Approval</div>`;
    ...
    function loadMaterial() {
        ...
        const cacheKey = `${course.value}_${level.value}_${sublevel.value}`;
        if (materialCache[cacheKey] !== undefined) { applyMaterial(materialCache[cacheKey]); return; }

        if (materialFetchController) materialFetchController.abort();
        materialFetchController = new AbortController();
        if (materialSection) materialSection.style.opacity = '0.5';

        fetch('/get-material', {
            method:'POST', signal:materialFetchController.signal,
            headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},
            body:JSON.stringify({ course_template_id:course.value, level_id:level.value, sublevel_id:sublevel.value })
        })
        .then(r=>r.json())
        .then(data => { materialCache[cacheKey]=data; if(materialSection) materialSection.style.opacity='1'; applyMaterial(data); })
...
function applyMaterial(data) {
    ...
    function loadPackages(courseId) {
        ...
        fetch(`/level-packages/${courseId}`)
            ...
            .then(packages => {
                ...
                const none = createPkgCard(null,null,null);
                none.classList.add('selected');
                none.innerHTML = `<div class="package-card-label">No Package</div><div class="package-card-levels">—</div><div class="package-card-price">Regular price</div><div class="package-card-check"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg></div>`;
                none.addEventListener('click', () => applyPackage(null));
                container.appendChild(none);

                packages.forEach(pkg => {
                    const card = createPkgCard(pkg.package_id, pkg.package_price, pkg.levels_count);
                    card.innerHTML = `<div class="package-card-label">Package</div><div class="package-card-levels">${pkg.levels_count} <span style="font-size:16px;">Levels</span></div><div class="package-card-price">${parseFloat(pkg.package_price).toFixed(2)} LE</div><div class="package-card-per">${(pkg.package_price/pkg.levels_count).toFixed(0)} LE / level</div><div class="package-card-check"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg></div>`;
    ...
    function createPkgCard(id, price, levels) {
    ...
    function applyPackage(pkg) {
        ...
        if (!pkg) {
            pricing.isPackage = false; pricing.packageId = null;
            if (pkgHidden) pkgHidden.value = '';
            if (notice)    notice.style.display = 'none';
            document.querySelector('.package-card')?.classList.add('selected');
            if (bundle) { bundle.disabled=false; bundle.style.opacity='1'; bundle.title=''; }
            calculatePrice();
            return;
        }
        ...
        if (notice) {
            notice.style.display='flex'; notice.style.gap='8px'; notice.style.alignItems='center';
    ...
    window.updatePaymentTotal = function() {
        ...
        let total=0; amounts.forEach(i=>{ total+=parseFloat(i.value||0); });
        ...
        if (required>0) {
            ...
            if (diff<0.01) { totalEl.className='payment-total-value success'; msgEl.className='payment-validation-msg success'; msgEl.textContent='✓ Payment amounts match the required deposit.'; }
            else if (total>required) { totalEl.className='payment-total-value error'; msgEl.className='payment-validation-msg error'; msgEl.textContent=`Amount (${total.toFixed(2)} LE) exceeds required (${required} LE).`; }
            else { totalEl.className='payment-total-value error'; msgEl.className='payment-validation-msg error'; msgEl.textContent=`Still need ${(required-total).toFixed(2)} LE more.`; }
    ...
    if (previewBtn) {
        previewBtn.addEventListener('click', async function() {
            ...
            if (!paymentSelect?.value) {
                await infConfirm.show({ label:'Validation', title:'Missing Payment Plan',
                    message:'Please select a payment plan before continuing.', okText:'OK' });
                return;
            }
            ...
            if (depositSection && depositSection.style.display !== 'none') {
                ...
                if (Math.abs(total - required) > 0.01) {
                    await infConfirm.show({ label:'Validation', title:'Deposit Incomplete',
                        message:`The deposit total (${total.toFixed(2)} LE) must equal the required amount (${required.toFixed(2)} LE).`, okText:'OK' });
                    return;
                }
            }
