...
.sc-nav-link:hover,.sc-nav-link.active{color:#1B4FA8;text-decoration:none;}
.sc-nav-link:hover::after,.sc-nav-link.active::after{width:100%;}

.sc-avatar{width:34px;height:34px;border-radius:50%;background:rgba(27,79,168,0.07);
    border:1.5px solid rgba(27,79,168,0.2);display:flex;align-items:center;justify-content:center;
    transition:border-color 0.25s;flex-shrink:0;cursor:pointer;}
.sc-avatar:hover{border-color:#1B4FA8;}

.sc-dropdown{display:none;position:absolute;right:0;top:calc(100% + 10px);
...
.sc-dropdown.open{display:block;animation:scDropIn 0.2s ease both;}
@keyframes scDropIn{from{opacity:0;transform:translateY(-6px)}to{opacity:1;transform:none}}

.sc-dropdown-item{display:flex;align-items:center;gap:8px;padding:10px 16px;font-size:10px;
...
.sc-dropdown-item:hover{color:#1B4FA8;background:rgba(27,79,168,0.04);text-decoration:none;}
.sc-dropdown-item.danger:hover{color:#DC2626;background:rgba(220,38,38,0.04);}

#scBellPanel{display:none;position:absolute;right:0;top:calc(100% + 10px);
...
.sc-bell-dot{position:absolute;top:5px;right:5px;width:7px;height:7px;
    border-radius:50%;background:#F5911E;border:1.5px solid #fff;display:none;}
.sc-bell-btn{background:none;border:none;cursor:pointer;padding:7px;color:#AAB8C8;
    position:relative;transition:color 0.2s;display:flex;align-items:center;}
.sc-bell-btn:hover{color:#F5911E;}

.sc-hamburger{background:none;border:none;cursor:pointer;padding:6px;display:none;
...
<div style="max-width:1600px;margin:0 auto;padding:0 20px;">
    <div style="display:flex;align-items:center;height:62px;gap:12px;">
        ...
        {{-- Logo --}}
        <a href="{{ route('student-care.dashboard') }}" style="text-decoration:none;flex-shrink:0;">
            <img src="{{ asset('images/logo.png') }}" alt="Infinity" style="height:34px;width:auto;display:block;">
        </a>

        {{-- Desktop Links --}}
        <div class="sc-nav-desktop-links" style="display:flex;align-items:center;gap:24px;margin-left:8px;flex:1;overflow:hidden;">
            <a href="{{ route('student-care.dashboard') }}"       class="sc-nav-link {{ request()->routeIs('student-care.dashboard') ? 'active' : '' }}">Dashboard</a>
            <a href="{{ route('student-care.waiting-list') }}"    class="sc-nav-link {{ request()->routeIs('student-care.waiting-list') ? 'active' : '' }}">Waiting List</a>
            <a href="{{ route('student-care.instances') }}"       class="sc-nav-link {{ request()->routeIs('student-care.instances*') ? 'active' : '' }}">Courses</a>
            <a href="{{ route('student-care.outstanding') }}"     class="sc-nav-link {{ request()->routeIs('student-care.outstanding') ? 'active' : '' }}">Outstanding</a>
            <a href="{{ route('student-care.postponed') }}"       class="sc-nav-link {{ request()->routeIs('student-care.postponed') ? 'active' : '' }}">Postponed</a>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:4px;margin-left:auto;flex-shrink:0;">
            ...
            {{-- Bell --}}
            <div style="position:relative;" id="scBellWrap">
                <button class="sc-bell-btn" onclick="scToggleBell()">
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/>
                        <path d="M13.73 21a2 2 0 0 1-3.46 0"/>
                    </svg>
                    <span class="sc-bell-dot" id="scBellBadge"></span>
                </button>
                ...
                <div id="scBellPanel">
                    ...
                    <div style="max-height:300px;overflow-y:auto;scrollbar-width:thin;scrollbar-color:rgba(27,79,168,0.1) transparent;">
                        @if(isset($navNotifications) && $navNotifications->count())
                            ...
                            @endphp
                            <a href="{{ $notif->url ?? '#' }}"
                               onclick="scMarkRead({{ $notif->user_notification_id }})"
            ...
            {{-- User menu --}}
            <div style="position:relative;" id="scUserWrap">
                <button onclick="scToggleUser()"
                    ...
                    <div class="sc-nav-desktop-name" style="display:flex;flex-direction:column;align-items:flex-start;line-height:1;">
                ...
                <div id="scUserMenu" class="sc-dropdown">
...
function scToggleBell(){
    ...
    if(!open)p.style.animation='scDropIn 0.2s ease both';
}

function scToggleUser(){
    document.getElementById('scUserMenu').classList.toggle('open');
    document.getElementById('scBellPanel').style.display='none';
}

document.addEventListener('click',(e)=>{
    if(!document.getElementById('scBellWrap')?.contains(e.target))
        document.getElementById('scBellPanel').style.display='none';
    if(!document.getElementById('scUserWrap')?.contains(e.target))
        document.getElementById('scUserMenu')?.classList.remove('open');
});

const scUnread={{isset($navUnreadCount)?(int)$navUnreadCount:0}};
if(scUnread>0)document.getElementById('scBellBadge').style.display='block';

async function scMarkRead(id){
...
const scPrevUnread={{$navPrevUnread??0}};
if(scUnread>scPrevUnread){
    try{
