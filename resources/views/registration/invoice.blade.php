...
.inf-modal{
    ...
    animation:infModalIn 0.35s cubic-bezier(0.16,1,0.3,1) both;overflow:hidden;
}
...
.inf-modal-status{font-size:9px;letter-spacing:2px;text-transform:uppercase;background:rgba(5,150,105,0.2);color:#4ADE80;padding:4px 10px;border-radius:20px;border:1px solid rgba(74,222,128,0.2);}
.inf-header-actions{display:flex;align-items:center;gap:8px;position:relative;z-index:1;flex-shrink:0;}
.inf-modal-close{width:34px;height:34px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:8px;cursor:pointer;color:rgba(255,255,255,0.5);transition:all 0.2s;pointer-events:auto;}
.inf-modal-close:hover{background:rgba(220,38,38,0.25);border-color:rgba(220,38,38,0.4);color:#fff;}
.inf-print-btn{display:flex;align-items:center;gap:6px;padding:8px 16px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:8px;cursor:pointer;color:rgba(255,255,255,0.7);font-family:'DM Sans',sans-serif;font-size:10px;letter-spacing:2px;text-transform:uppercase;transition:all 0.2s;pointer-events:auto;}
.inf-print-btn:hover{background:rgba(255,255,255,0.15);color:#fff;}

.inf-modal-body{flex:1;overflow-y:auto;overflow-x:hidden;scroll-behavior:smooth;scrollbar-width:thin;scrollbar-color:rgba(27,79,168,0.15) transparent;}
...
.inf-modal-footer{padding:14px 28px;border-top:1px solid rgba(27,79,168,0.08);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;background:#fff;box-shadow:0 -4px 20px rgba(27,79,168,0.06);border-radius:0 0 16px 16px;}
...
.btn-inf-back{padding:10px 20px;background:transparent;border:1px solid rgba(27,79,168,0.15);border-radius:6px;color:#7A8A9A;font-family:'DM Sans',sans-serif;font-size:11px;letter-spacing:2px;text-transform:uppercase;cursor:pointer;transition:all 0.2s;pointer-events:auto;}
.btn-inf-back:hover{border-color:rgba(27,79,168,0.3);color:#1B4FA8;}
.btn-inf-confirm{display:inline-flex;align-items:center;gap:8px;padding:11px 26px;background:linear-gradient(135deg,#1B4FA8,#2D6FDB);border:none;border-radius:6px;color:#fff;font-family:'Bebas Neue',sans-serif;font-size:15px;letter-spacing:4px;cursor:pointer;transition:all 0.25s;box-shadow:0 4px 16px rgba(27,79,168,0.3);pointer-events:auto;}
...
@media print{
    ...
    #inf-print-area{position:static!important;width:100%!important;font-family:'DM Sans',sans-serif!important;zoom:0.88;}

    #inf-print-area .inf-modal{position:static!important;width:100%!important;max-height:none!important;box-shadow:none!important;animation:none!important;border-radius:0!important;display:block!important;overflow:visible!important;}

    /* Header — dark minimal */
    #inf-print-area .inf-modal::before{display:none!important;}
    #inf-print-area .inf-modal-footer,
    #inf-print-area .inf-header-actions,
    #inf-print-area .inf-print-btn{display:none!important;}
    ...
    #inf-print-area .inf-modal-eyebrow{color:rgba(255,255,255,0.5)!important;font-size:8px!important;}
    #inf-print-area .inf-modal-title{font-size:22px!important;letter-spacing:4px!important;color:#fff!important;}
    #inf-print-area .inf-modal-id{color:rgba(255,255,255,0.85)!important;border-color:rgba(255,255,255,0.2)!important;font-size:9px!important;}
    #inf-print-area .inf-modal-date{color:rgba(255,255,255,0.4)!important;font-size:9px!important;}
    #inf-print-area .inf-modal-status{color:rgba(255,255,255,0.7)!important;background:transparent!important;border:none!important;font-size:8px!important;}

    #inf-print-area .inf-modal-body{overflow:visible!important;max-height:none!important;}

    /* Two col */
    #inf-print-area .inf-two-col{display:grid!important;grid-template-columns:1fr 1fr!important;border-bottom:1px solid #E5E7EB!important;}
    #inf-print-area .inf-col-box{padding:8px 12px!important;border-right:1px solid #E5E7EB!important;}
    #inf-print-area .inf-col-box:last-child{border-right:none!important;}

    /* Sections */
    #inf-print-area .inf-section{padding:8px 12px!important;border-bottom:1px solid #E5E7EB!important;}
    #inf-print-area .inf-section:last-child{border-bottom:none!important;}

    /* Labels */
    #inf-print-area .inf-sec-label{color:#374151!important;font-size:7px!important;margin-bottom:6px!important;}
    #inf-print-area .inf-sec-label::after{background:#E5E7EB!important;}

    /* Rows */
    #inf-print-area .inf-row{padding:3px 0!important;border-bottom:1px solid #F3F4F6!important;}
    #inf-print-area .inf-key{font-size:9px!important;color:#6B7280!important;}
    #inf-print-area .inf-val{font-size:10px!important;color:#111827!important;}
    #inf-print-area .inf-val.blue,
    #inf-print-area .inf-val.orange,
    #inf-print-area .inf-val.green,
    #inf-print-area .inf-val.red{color:#111827!important;}

    /* Pricing table */
    #inf-print-area .inf-pricing-table th{font-size:7px!important;padding:4px 8px!important;background:#F9FAFB!important;color:#6B7280!important;border-bottom:1px solid #D1D5DB!important;}
    #inf-print-area .inf-pricing-table td{font-size:9px!important;padding:4px 8px!important;color:#111827!important;border-bottom:1px solid #F3F4F6!important;}
    #inf-print-area .inf-pricing-table td:last-child{color:#111827!important;font-weight:600!important;}
    #inf-print-area .inf-price-tag{background:#F3F4F6!important;color:#374151!important;font-size:7px!important;}

    /* Totals — dark strip preserved */
    ...
    #inf-print-area .inf-total-item{border-right:1px solid rgba(255,255,255,0.1)!important;padding:8px 10px!important;}
    #inf-print-area .inf-total-label{color:rgba(255,255,255,0.5)!important;font-size:7px!important;}
    #inf-print-area .inf-total-val{font-size:16px!important;color:#fff!important;}
    #inf-print-area .inf-total-val.orange,
    #inf-print-area .inf-total-val.green{color:#fff!important;}
    #inf-print-area .inf-total-sub{color:rgba(255,255,255,0.35)!important;font-size:8px!important;}

    /* Plan */
    #inf-print-area .inf-plan-header{background:#F9FAFB!important;border:1px solid #E5E7EB!important;padding:6px 10px!important;margin-bottom:6px!important;}
    #inf-print-area .inf-plan-name{color:#111827!important;font-size:11px!important;}
    #inf-print-area .inf-plan-badge{background:#F3F4F6!important;color:#374151!important;border:1px solid #D1D5DB!important;font-size:7px!important;}

    /* Inst table */
    #inf-print-area .inf-inst-table th{font-size:7px!important;padding:4px 7px!important;background:#F9FAFB!important;color:#6B7280!important;}
    #inf-print-area .inf-inst-table td{font-size:9px!important;padding:4px 7px!important;color:#111827!important;}
    #inf-print-area .inf-inst-table td:nth-child(2){color:#111827!important;}
    #inf-print-area .inf-inst-table td:last-child{color:#374151!important;}

    /* Terms */
    #inf-print-area .inf-terms-list li{font-size:8px!important;color:#6B7280!important;}
    #inf-print-area .inf-terms-dot{background:#374151!important;}

    /* Footer watermark */
    #inf-print-area::after{
        content:'Infinity Academy — Official Invoice';
        ...
        <div class="inf-modal-header">
            <div style="position:relative;z-index:1;">
                ...
                <div class="inf-modal-meta">
                    ...
                    <div class="inf-modal-status">CS: {{ Auth::user()->employee?->full_name ?? Auth::user()->name }}</div>
...
document.getElementById('invoiceModal').addEventListener('click', function(e) { if (e.target === this) infCloseModal(); });
...
/* ── Confirm & Register ── */
document.getElementById('confirm_register_btn').addEventListener('click', async function() {
    const btn = this;
    btn.disabled = true;
    btn.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 1s linear infinite"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg><span>Processing...</span>';
    ...
    try {
        ...
        if (data.success) {
            document.getElementById('inv_ref').textContent =
                'INV-' + data.enrollment_id.toString().padStart(6, '0');
...
window.infPrintInvoice = function() {
    ...
    if (schedSection && schedSection.style.display === 'none') schedSection.remove();
    printArea.innerHTML = '';
...
window.buildInvoice = function() {
    ...
    if (depositSec && depositSec.style.display !== 'none') {
        const methods = [...document.querySelectorAll('.payment-method-row')]
            .map(row => ({ method: row.querySelector('.method-select')?.value || '—', amount: parseFloat(row.querySelector('.method-amount')?.value || 0) }))
            .filter(m => m.amount > 0);
        if (methods.length) {
            payHTML += `<div style="margin-top:16px;"><div class="inf-sec-label" style="margin-bottom:10px;">Deposit Payment Methods</div>`;
            methods.forEach(m => { payHTML += infRow(m.method.replace('_',' '), fmtLE(m.amount), 'green'); });
            payHTML += `<div style="height:1px;background:rgba(27,79,168,0.07);margin:6px 0;"></div>`;
            payHTML += infRow('Total Paid Now', fmtLE(methods.reduce((s,m)=>s+m.amount,0)), 'green');
            payHTML += `</div>`;
        }
    }
