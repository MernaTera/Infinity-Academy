...
.period-tab:hover:not(.active){color:var(--blue);background:var(--blue-l);text-decoration:none;}

/* ── ALERTS ── */
.alert-strip{display:flex;flex-direction:column;gap:8px;margin-bottom:20px;}
.alert-item{display:flex;align-items:center;gap:10px;padding:11px 16px;border-radius:6px;font-size:12px;}
...
.kpi-card{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:18px 20px;position:relative;overflow:hidden;transition:all 0.3s;}
...
@media(max-width:768px){
    ...
    .pm-4-grid{grid-template-columns:1fr 1fr!important;}
    .qa-grid{grid-template-columns:repeat(3,1fr);}
}
@media(max-width:480px){
    .kpi-grid-4,.kpi-grid-3,.kpi-grid-5,.kpi-grid-2{grid-template-columns:1fr;}
    .pm-4-grid{grid-template-columns:1fr!important;}
...
<div class="adm-dash">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="dash-sub">
                {{ now()->format('l, d M Y') }} ·
                Showing: <strong>{{ match($period) { 'day'=>'Today','week'=>'This Week','month'=>'This Month','patch'=>'Current Patch',default=>'All Time' } }}</strong>
            </p>
        </div>
    ...
    <div class="patch-banner">
        ...
        <div style="flex:1;max-width:260px;position:relative;z-index:1">
            ...
            <div class="pb-prog-track"><div class="pb-prog-fill" style="width:{{ $pPct }}%"></div></div>
        ...
        <div style="display:flex;gap:28px;flex-wrap:wrap;position:relative;z-index:1">
            <div style="text-align:center"><div class="pb-stat-val">{{ $activeCourses }}</div><div class="pb-stat-label">Courses</div></div>
            <div style="text-align:center"><div class="pb-stat-val">{{ $totalStudents }}</div><div class="pb-stat-label">Students</div></div>
            <div style="text-align:center"><div class="pb-stat-val">{{ number_format($patchRevenue/1000,1) }}K</div><div class="pb-stat-label">Revenue LE</div></div>
            <div style="text-align:center"><div class="pb-stat-val">{{ $targetPct }}%</div><div class="pb-stat-label">CS Target</div></div>
        </div>
    </div>
    @endif
    ...
    {{-- ── REVENUE KPIs ── --}}
    <span class="sec-label">Financial Overview — {{ match($period){'day'=>'Today','week'=>'This Week','month'=>'This Month','patch'=>'Current Patch',default=>'All Time'} }}</span>
    <div class="kpi-grid kpi-grid-4">
        <div class="kpi-card" style="--kc:var(--blue);--ki:var(--blue-l)">
            ...
            <div class="kpi-sub">LE — {{ match($period){'day'=>'today','week'=>'this week','month'=>'this month','patch'=>'current patch',default=>'all time'} }}</div>
        </div>
        ...
        <div class="kpi-card" style="--kc:var(--red);--ki:var(--red-l)">
            ...
            <div class="kpi-sub">LE unpaid — all enrollments</div>
        ...
        <div class="kpi-card" style="--kc:var(--muted);--ki:rgba(122,138,154,0.1)">
            ...
            <div class="kpi-sub">LE — period</div>
        </div>
    </div>

    {{-- ── PAYMENT METHODS — 4 cards, no duplicates ── --}}
    <span class="sec-label">Payment Method Breakdown</span>
    @php $pmTotal = $cashRevenue + $instapayRevenue + $vodafoneRevenue + $cardRevenue; @endphp
    <div class="pm-4-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px;">
        ...
        <div class="pm-card" style="--pc:#059669;--pi:var(--green-l)">
            ...
            <div class="pm-sub">{{ $cashCount }} transactions · LE</div>
        ...
        <div class="pm-card" style="--pc:#7F77DD;--pi:var(--purple-l)">
            ...
            <div class="pm-sub">{{ $instapayCount }} transactions · LE</div>
        ...
        <div class="pm-card" style="--pc:#E11D48;--pi:rgba(225,29,72,0.08)">
            ...
            <div class="pm-sub">{{ $vodafoneCount }} transactions · LE</div>
        ...
        <div class="pm-card" style="--pc:var(--teal);--pi:var(--teal-l)">
            <div class="pm-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#0891B2" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg></div>
            <div class="pm-name">Card</div>
            <div class="pm-val">{{ number_format($cardRevenue) }}</div>
            <div class="pm-sub">{{ $cardCount }} transactions · LE</div>
            <div class="pm-bar"><div class="pm-bar-fill" style="width:{{ $pmTotal>0?round($cardRevenue/$pmTotal*100):0 }}%;background:#0891B2"></div></div>
            <div style="font-size:9px;color:var(--faint);margin-top:4px">{{ $pmTotal>0?round($cardRevenue/$pmTotal*100):0 }}% of total</div>
        </div>
    ...
    <div class="dash-grid-3" style="margin-bottom:24px;">
        ...
        <div class="card">
            ...
            <div class="chart-wrap" style="height:200px;"><canvas id="enrollChart"></canvas></div>
        </div>
    </div>
    ...
    <div class="dash-grid">
            ...
            <div class="card" style="margin-bottom:20px;">
                ...
                <div class="card-body">
                    ...
                    <div class="rev-row">
                        ...
                        <div class="rev-bar-track"><div class="rev-bar-fill" style="width:{{ $maxRev>0?round($rc->total/$maxRev*100):0 }}%"></div></div>
                        <div style="font-family:'Bebas Neue',sans-serif;font-size:15px;color:var(--blue);letter-spacing:1px;white-space:nowrap;margin-left:8px;">{{ number_format($rc->total/1000,1) }}K</div>
            ...
            <div class="card" style="margin-bottom:20px;">
                ...
                <div class="card-body">
                    ...
                    <div class="rev-row">
                        ...
                        <div style="font-family:'Bebas Neue',sans-serif;font-size:15px;color:var(--orange);letter-spacing:1px;white-space:nowrap;margin-left:8px;">{{ number_format($rb->total/1000,1) }}K</div>
                    </div>
                    @endforeach
                </div>
            </div>
        ...
        <div>
            <div class="card" style="margin-bottom:16px;">
                <div class="card-header"><div class="card-title">Workforce & Operations</div></div>
            ...
            <div class="card">
                ...
                @php
                    $courseName = $enr->courseTemplate?->name
                ...
                <div class="enr-row">
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        <div style="font-size:10px;color:var(--faint);margin-top:1px;">{{ $courseName }}</div>
                    </div>
...
const chartDefaults = {
    ...
    plugins:{ legend:{display:false}, tooltip:{
        backgroundColor:'#1A2A4A', titleColor:'#AAB8C8', bodyColor:'#fff',
        padding:10, cornerRadius:6,
        titleFont:{size:10,family:'DM Sans'}, bodyFont:{size:13,family:'Bebas Neue'}
    }},
    scales:{
...
new Chart(document.getElementById('revenueChart'),{
    ...
    data:{
        ...
        datasets:[{
            ...
            borderColor:'#1B4FA8', borderWidth:2, borderRadius:4,
...
new Chart(document.getElementById('enrollChart'),{
    ...
    data:{
        ...
        datasets:[{
            ...
            borderColor:'#F5911E', backgroundColor:'rgba(245,145,30,0.08)',
            borderWidth:2, tension:0.4, fill:true,
            pointBackgroundColor:'#F5911E', pointRadius:3, pointHoverRadius:5,
        }]
    },
